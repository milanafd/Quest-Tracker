...
def load_quests():
    ...
    # Check if today's folder has .json quest files
    ...
    if today_has_quests:
        ...
        # Find the most recent previous day with quests
                ...
                if filename.endswith(".json"):
                    with open(os.path.join(latest_dir, filename), "r") as f:
                        ...
                        if task_data['status'] != "Quest completed":
                            tasks.append(task_data)
                            task_id_counter = max(task_id_counter, task_data['id'] + 1)
            save_quests()  # Save them into today's folder
...
def update_log():
    ...
    if selected_task_index:
        ...
        if log_entry:
            ...
            # Save log in the individual quest's log file
